// src/components/Watches/Watches.jsx - UPDATED FOR API
...
const Watches = ({ title = "Watch Collection", autoplayDelay = 3500, gender = "women" }) => {
  // Fetch watches from API with gender filter
  const { products: watchesData, loading, error } = useProductsByCategory('watches', { gender });
  
  // Extract unique brands
  const uniqueWatchBrands = useBrands(watchesData);

  if (loading) {
    return (
      <section className="watches py-16 bg-gray-100 dark:bg-gray-900">
  ...
  if (error) {
    return (
      <section className="watches py-16 bg-gray-100 dark:bg-gray-900">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-8">{title}</h2>
          <p className="text-red-600">Error loading watches: {error}</p>
        </div>
      </section>
    );
  }
  ...
  if (watchesData.length === 0) {
    return (
      <section className="watches py-16 bg-gray-100 dark:bg-gray-900">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-8">{title}</h2>
          <p className="text-gray-600 dark:text-gray-400">No watches available</p>
        </div>
      </section>
    );
  }
              ...
              <SwiperSlide key={watch.id}>
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-gray-200 mb-2">${watch.price}</p>
                    <p className="text-gray-300 text-sm mb-4">{watch.code}</p>
